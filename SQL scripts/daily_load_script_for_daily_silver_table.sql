...
--Purpose of script
==================================================================================================================
--1. Deduplicates bronze daily price and volume data table and loads only new data into the silver table.

--2. Deduplicates silver layer.
==================================================================================================================

  --Dedupliating bronze layer
CREATE OR REPLACE TABLE btc_historical_exchange_data.daily_crypto_price_data_bronze_cointinuous_load_raw AS   
                            ...
                            from btc_historical_exchange_data.daily_crypto_price_data_bronze_cointinuous_load_raw
                            ) A
                            WHERE RN = 1;
      ...
      --Inserting only new records into silver layer.                       
                          INSERT INTO btc_historical_exchange_data.daily_crypto_price_data_silver_layer (
                UNIT,
                TIMESTAMP,
                TYPE,
                MARKET,
              ...
              CAST(CON.LOW AS NUMERIC),
                ...
                CON.COIN_PAIR
                                FROM btc_historical_exchange_data.daily_crypto_price_data_bronze_cointinuous_load_raw CON
                                LEFT JOIN btc_historical_exchange_data.daily_crypto_price_data_silver_layer SIL
                                ON 
...
--Deduplicating silver layer
                             CREATE OR REPLACE TABLE  btc_historical_exchange_data.daily_crypto_price_data_silver_layer AS   
                            SELECT * EXCEPT (RN) FROM (
                            SELECT *,
